...
class Jquery{
	...
	function find($par){
		...
		foreach ($this->doms as $temj){
			$res = array_merge($res,$temj->find($par));
		}
	 ...
	 */
	function parent(){
		if($this->isroot||$this->dcount!=1){
			return new Jquery();
		}else{
			$parent = $this->doms[0]->parent();
			if(isset($parent)){
	...
	function next(){
		$res = array();
		foreach ($this->doms as $temj){
			if($temj->next_sibling()!=null)
				$res[] = $temj->next_sibling();				
		}
		return new jQuery($res);
	}
	...
	function prev(){
		$res = array();
		foreach ($this->doms as $temj){
			if($temj->prev_sibling()!=null)	
				$res[] = $temj->prev_sibling();				
		}
		return new jQuery($res);
	}
	...
	function get($index){
		if($index===-1){
			return $this->doms[$this->dcount-1];
		}else{
	...
	function eq($index){
		if($index===-1){
			return new jQuery($this->doms[$this->dcount-1]);
		}else{
			return new jQuery($this->doms[$index]);
		}
	}
	/**
	 * 获得/设置所含元素的html外部代码
	 */
	function ohtml($pam=null){
		$res = array();
		foreach ($this->doms as $temj){
			if($pam!==null){
				$temj->outertext = $pam;
	...
	function is($par){
		if($this->isroot){
			...
			$parsetemprootjq = $temprootjq->find($par)->eq(0);
			if($this->ohtml()==$parsetemprootjq->ohtml()){
				return true;
			}
			return false;
		}
		foreach ($this->doms as $temj){
			...
			$parsetemprootjq = $temprootjq->find($par)->eq(0);
			if($temj->ohtml()!=$parsetemprootjq->ohtml()){
				return false;
			}
		}
		return true;
	...
	function text(){
		$res = array();
		foreach ($this->doms as $temj){
			$res[] =preg_replace('/\s\s+/', " ", str_replace('　', ' ',str_ireplace('&nbsp;', ' ',strip_tags($temj->plaintext))));
		}
		return trim(implode("\n", $res));
	}
	/**
